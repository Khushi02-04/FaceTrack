...
const getStudentMeta = (apiStudent: StudentApiResponse) => {
  const metaDocument = apiStudent.documents?.find((document) => document?.type === 'student_meta')

  return {
    department:
      typeof apiStudent.department === 'string' && apiStudent.department.trim()
        ? apiStudent.department
        : typeof metaDocument?.department === 'string' && metaDocument.department.trim()
          ? metaDocument.department
          : undefined,
    yearOfStudy:
      typeof apiStudent.year_of_study === 'string' && apiStudent.year_of_study.trim()
        ? apiStudent.year_of_study
...
const mapApiStudentToStudent = (apiStudent: StudentApiResponse): Student => {
  ...
  const meta = getStudentMeta(apiStudent)
